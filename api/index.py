...
async def create_job(request: dict, background_tasks: BackgroundTasks):
    """Create a new job and start the Seekan-LG agent"""
    try:
        ...
        # Start Seekan-LG agent in background
        agent = SeekanLGAgent(job_id, campaign["config"])
        background_tasks.add_task(run_seekanlg_agent, agent, planned_count, job_id)
        
        logger.info(f"Seekan-LG created job: {job_id} for campaign: {campaign_id}")
...
async def run_seekanlg_agent(agent: SeekanLGAgent, planned_count: int, job_id: str):
    """Run Seekan-LG agent in background"""
    try:
        result = await agent.run_agent_loop(planned_count)
        if result["status"] == "completed":
            jobs_db[job_id]["status"] = "completed"
            jobs_db[job_id]["sentCount"] = result["drafts_created"]
            logger.info(f"Seekan-LG job {job_id} completed with {result['drafts_created']} drafts")
        else:
            jobs_db[job_id]["status"] = "failed"
            logger.error(f"Seekan-LG job {job_id} failed: {result.get('error')}")
    except Exception as e:
        jobs_db[job_id]["status"] = "failed"
        logger.error(f"Seekan-LG agent crashed for job {job_id}: {e}")
